...
async function getAuctionItems(formattedAuctionId, deviceId) {
  ...
  try {
    ...
    const formattedItems = await Promise.all(itemsData.activeItems.map(async (gem) => {
      ...
      if (auctionMode === "OPEN" || auctionMode === "ENGLISH") {
...
async function placeBid(bidData, auctionUuid) {
  console.log("   Placing bid:", JSON.stringify(bidData, null, 2));
  try {
    const bidResult = await placeBidDB(bidData);
  ...
  try {
    ...
    // ---------- GET ITEMS ----------
      ...
      if (!formattedAuctionId) {
        ...
        const itemsData = await getAuctionItems(formattedAuctionId, deviceId);

        if (!itemsData) {
          response.Status = "FAILED";
          response.Reason = 2; // Auction not found or failure
        } else {
