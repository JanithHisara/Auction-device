...
// Helper functions
// We now rely on auction.auction_code from the DB instead of formatting an index.
...
export async function updateAuctionCache(fetchAuctionsFn) {
  ...
  auctions.forEach((auction, index) => {
    const formattedId = auction.auction_code || `AUC-${auction.id.substring(0,6).toUpperCase()}`;
    formattedToUuid[formattedId] = auction.id;
    uuidToFormatted[auction.id] = formattedId;
    codeToUuid[formattedId] = auction.id;
