...
async function getGemUuid(auctionId, formattedItemId) {
  console.log(`   Looking up gem: Auction ${auctionId}, Item ${formattedItemId}`);
  try {
    const auctionCache = await getAuctionCache(fetchAuctions);
    const auctionUuid = auctionCache.codeToUuid[auctionId];

    if (!auctionUuid) {
      console.log(`❌ Auction ID not found: ${auctionId}`);
      return null;
    }

    const itemsData = await getActiveItems(auctionUuid, auctionId, fetchActiveItems);

    if (!itemsData || itemsData.count === 0) {
      console.log(`❌ No active items for auction: ${auctionId}`);
      return null;
    }
...
async function getAuctionItems(auctionId, userId = null) {
  console.log(`   Fetching items for auction: ${auctionId}`);
  try {
    const cache = await getAuctionCache(fetchAuctions);
    const auctionUuid = cache.codeToUuid[auctionId];
        ...
        if (result.valid) {
          ...
          if (auctionId) {
            ...
            if (auctionUuid) {
              ...
              isRegistered = true;
              console.log("Bypassing strict registration check for auction:", auctionId);
            }
          }
    ...
    // ---------- SUBMIT BID ----------
      ...
      if (!nfcResult.valid) {
        ...
        if (!isRegistered) {
          ...
          const gemUuid = await getGemUuid(auctionId, formattedItemId);
